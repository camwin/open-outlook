# OpenOutlook

**Modern, open-source email & calendar client in the spirit of Outlook 2003**  
A free, minimalist alternative to Microsoft Outlook that works offline-first and supports multiple accounts.

[![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0)


## ✨ Features

- Unified inbox for multiple accounts (IMAP, POP3, Microsoft Exchange/365 via EAS or IMAP, Gmail, iCloud, etc.)
- Built-in calendar, tasks, and contacts with CalDAV/CardDAV support
- Focused Inbox (smart sorting of important vs. other mail)
- Offline access & full email caching
- End-to-end encryption support (PGP/OpenPGP & S/MIME)
- Clean, modern UI
- Keyboard shortcuts inspired by Outlook + Gmail
- Rules & filters (client-side)
- Search that actually works (full-text + fast indexing)
- Cross-platform: Windows, macOS, Linux (desktop)